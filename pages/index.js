  ...
  return (
    <div className="bg-gray-500">
      <div className={styles.container}>
        <Head>
          <title>AptoPunks | Mint</title>
          <meta name="description" content="Mint your AptoPunks" />
          <link rel="icon" href="/favicon.png" />
        </Head>

        <main className={styles.main}>
          <h1 className={styles.title}>
          <a href="https://punkedaptos.com"><img src="/logo.png"></img></a>
          </h1>
          <div className={styles.topcorner}>
            <ConnectWalletButton connectButton={!wallet.connected} className="d-flex" />
          </div>
          <br></br>
          <img src="/ex.gif" style={{ width: "336px", height:"336px", borderRadius:"25px",maxWidth:"80%" }} />
          <div id="collection-info" className="d-flex flex-column align-items-center text-white" style={{width: "80%"}}>
            ...
            <>
              <div className="d-flex align-items-center my-3">
                <input style={{width:"50px", fontSize:"25px", borderRadius:"5px"}} className={`${styles.defaultInput} me-3`} type="number" min="1" max={candyMachineData.data.maxMintsPerWallet} />
                <button className={styles.button} style={{width:"100px", fontSize:"33px"}} onClick={mint} disabled={!canMint}>{mintInfo.minting ? <Spinner animation="border" role="status"><span className="visually-hidden">Loading...</span></Spinner> : "Mint"}</button>
              ...
              <h5>{COLLECTION_SIZE - candyMachineData.data.numMintedTokens } Remaining</h5>
              <h3 ><span style={{color:"#d60404"}}>{candyMachineData.data.mintFee * mintInfo.numToMint}</span> APT per  AptoPunk</h3>
              <div className="d-flex flex-column align-items-center my-3">
                <h3 style={{textDecoration: "underline"}}></h3>
                <h6>{timeLeftToMint.presale === "LIVE" ? "" : timeLeftToMint.presale.days + " days : " + timeLeftToMint.presale.hours + " hours : " + timeLeftToMint.presale.minutes + " minutes : " + timeLeftToMint.presale.seconds + " seconds"}</h6>
              </div>
              <div className="d-flex flex-column align-items-center my-3">
                <h3 style={{textDecoration: "underline"}}></h3>
                <h6 style={{ fontSize:"20px", color:"#d60404"}}>{timeLeftToMint.public === "LIVE" ? "MINT LIVE" : timeLeftToMint.public.days + " days : " + timeLeftToMint.public.hours + " hours : " + timeLeftToMint.public.minutes + " minutes : " + timeLeftToMint.public.seconds + " seconds"}</h6>
              </div>
            </>}
          </div>
